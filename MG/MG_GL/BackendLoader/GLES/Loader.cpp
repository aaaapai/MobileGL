...
namespace MG_GL::BackendLoader {
    void Init() {
        GLESBackend::LoadLibs();
        GLESBackend::InitEGL();
        GLESBackend::InitGLES();
        GLESBackend::DestroyTempEGLCtx();
    }
}

namespace MG_GL::BackendLoader::GLESBackend {
    ...
    void LogOpenGLExtensions() {
        ...
        MG_Util::Debug::LogD("Extensions list using glGetString:\n%s",
              raw_extensions ? (const char *) raw_extensions : "(nullptr)");
        GLint num_extensions = 0;
        glGetIntegerv(GL_NUM_EXTENSIONS, &num_extensions);
        MG_Util::Debug::LogD("Extensions list using glGetStringi:\n");
        for (GLint i = 0; i < num_extensions; ++i) {
            const GLubyte *extension = glGetStringi(GL_EXTENSIONS, i);
            if (extension) {
                MG_Util::Debug::LogD("%s", (const char *) extension);
            } else {
                MG_Util::Debug::LogD("(nullptr)");
            }
        }
    }

    void InitGLESCapabilities() {
        GLES::glGetIntegerv(GL_MAJOR_VERSION, &GLES::Caps::Version::Major);
        GLES::glGetIntegerv(GL_MINOR_VERSION, &GLES::Caps::Version::Minor);
        
        GLint num_es_extensions = 0;
        GLES::glGetIntegerv(GL_NUM_EXTENSIONS, &num_es_extensions);
        MG_Util::Debug::LogD("Detected %d OpenGL ES extensions.", num_es_extensions);
        ...
        MG_Util::Debug::LogD("Detected GL_QCOM_texture_lod_bias!");
        
        if (GLES::Caps::Exts::GL_EXT_buffer_storage) {
            // TODO: Complete AppendExtension.
//AppendExtension("GL_ARB_buffer_storage");
        }

        if (GLES::Caps::Exts::GL_EXT_disjoint_timer_query) {
            // TODO: Complete AppendExtension.
...
//    INIT_GLES_FUNC(glTexStorage1D)
        ...
        MG_Util::Debug::LogD("glMultiDrawArraysIndirectEXT() @ 0x%x", GLES::glMultiDrawArraysIndirectEXT);
        MG_Util::Debug::LogD("glMultiDrawElementsIndirectEXT() @ 0x%x", GLES::glMultiDrawElementsIndirectEXT);
        MG_Util::Debug::LogD("glMultiDrawElementsBaseVertexEXT() @ 0x%x", GLES::glMultiDrawElementsBaseVertexEXT);
...
static EGL::name##_PTR egl_##name = NULL;                                   \
{                                                                           \
    static bool first = true;                                               \
    if (first) {                                                            \
        first = false;                                                      \
        if (libEGL != NULL) {                                               \
            egl_##name = (EGL::name##_PTR)ProcAddress(libEGL, #name);       \
        }                                                                   \
    }                                                                       \
}
    ...
    void InitEGL() {
        ...
        if (eglDisplay == EGL_NO_DISPLAY) {
            MG_Util::Debug::LogE("eglGetDisplay failed (0x%x)", egl_eglGetError());
            goto cleanup;
        }

        if (egl_eglInitialize(eglDisplay, NULL, NULL) != EGL_TRUE) {
            MG_Util::Debug::LogE("eglInitialize failed (0x%x)", egl_eglGetError());
            goto cleanup;
        }

        if (egl_eglBindAPI(EGL_OPENGL_ES_API) != EGL_TRUE) {
            MG_Util::Debug::LogE("eglBindAPI failed (0x%x)", egl_eglGetError());
            goto cleanup;
        }

        if (egl_eglChooseConfig(eglDisplay, configAttribs, &pbufConfig, 1, &configsFound) != EGL_TRUE) {
            MG_Util::Debug::LogE("eglChooseConfig failed (0x%x)", egl_eglGetError());
            goto cleanup;
        }
        ...
        if (configsFound == 0) {
            configAttribs[6] = 0;
            if (egl_eglChooseConfig(eglDisplay, configAttribs, &pbufConfig, 1, &configsFound) != EGL_TRUE) {
                MG_Util::Debug::LogE("Retry eglChooseConfig failed (0x%x)", egl_eglGetError());
                goto cleanup;
            }
            if (configsFound) {
                MG_Util::Debug::LogD("Using config without alpha channel");
            } else {
                MG_Util::Debug::LogE("No valid EGL config found");
                goto cleanup;
            }
        }
        ...
        if (eglContext == EGL_NO_CONTEXT) {
            MG_Util::Debug::LogE("eglCreateContext failed (0x%x)", egl_eglGetError());
            goto cleanup;
        }

        eglSurface = egl_eglCreatePbufferSurface(eglDisplay, pbufConfig, pbAttribs);
        if (eglSurface == EGL_NO_SURFACE) {
            MG_Util::Debug::LogE("eglCreatePbufferSurface failed (0x%x)", egl_eglGetError());
            goto cleanup;
        }

        if (egl_eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext) != EGL_TRUE) {
            MG_Util::Debug::LogE("eglMakeCurrent failed (0x%x)", egl_eglGetError());
            goto cleanup;
        }

        MG_Util::Debug::LogD("EGL initialized successfully");
        return;
        ...
        MG_Util::Debug::LogE("EGL initialization failed");
    }
