...
namespace MG_Util::Debug {
    ...
    if (SUCCEEDED(GetThreadDescription(GetCurrentThread(), &description)) {
        ...
        return buffer[0] ? buffer : "Unknown Thread";
    }
    ...
    void LogImpl(LevelType logLevel,
                 const char* levelTag,
                 const char* format,
                 int androidLevel,
                 va_list args) {
        if (MG_Global::Common::LogLevel > logLevel) return;

        std::string header = "[" + GetCurrentTime() + "] [" +
                             GetOSName() + " " +
                             GetCurrentThreadName() + "/" + levelTag + "]: ";
...
void Log##name(const char* format, ...) { \
    ...
    DECLARE_LOG_FUNCTION(W, WARN)
    DECLARE_LOG_FUNCTION(E, ERROR)
    DECLARE_LOG_FUNCTION(I, INFO)
    DECLARE_LOG_FUNCTION(F, FATAL)
...
#undef DECLARE_LOG_FUNCTION
    ...
    void LogClear() {
        ...
        logFile = fopen(MG_Global::Common::LOG_FILE_PATH, "w");
        if (logFile == nullptr) {
            return;
        }
        fclose(logFile);
